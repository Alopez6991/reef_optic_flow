...
class Optic_Flow_Calculator:
    def __init__(self, topic):
        ...
        self.optic_flow_pub_mean_x = rospy.Publisher("optic_flow_mean_vx", Float32, queue_size=10)
        self.optic_flow_pub_mean_y = rospy.Publisher("optic_flow_mean_vy", Float32, queue_size=10)
    ...
    def image_callback(self,image):
        try: # if there is an image
            ...
            self.optic_flow_pub_mean_x.publish(Float32(np.mean(flow[:,0,0])*2.0/320.0))
            self.optic_flow_pub_mean_y.publish(Float32(np.mean(flow[:,0,1])*2.0/320.0))
